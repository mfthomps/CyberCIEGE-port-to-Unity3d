...
public class ScrollingTextScript : MonoBehaviour {
  ...
  // ********************************************************************
  [SerializeField] private int m_numRepeats;
  [SerializeField] private bool m_repeatWhenEmpty = true;
  [SerializeField] private bool m_autoRemove;
  ...
  private readonly Dictionary<int, string> handle_dict = new Dictionary<int, string>();

  // ********************************************************************
  // Private Data Members 
  // ********************************************************************
  private int m_currentNumRepeats;
  ...
  private void UseNextMessage(bool new_message = false) {
    ...
    if (!(m_repeatWhenEmpty && m_messages.Count <= 1)) loadNew = true;
  ...
  public void RemoveMessage(string newMessage) {
    ...
    if (m_textMesh.text == newMessage) m_textMesh.text = "";
  }
  ...
  public void AddTicker(string xml) {
    ...
    int handle_int = -1;
    if (!int.TryParse(handle, out handle_int)) Debug.Log("Error: AddTicker parsing handle from " + handle);
